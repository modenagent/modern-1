...
class SetaPDF_Core_IccProfile_Parser
{
    ...
    public function __construct($reader)
    {
        if (!$reader instanceof SetaPDF_Core_Reader_Binary) {
            $fileReader = $reader;
            if (!$fileReader instanceof SetaPDF_Core_Reader_Abstract) {
    ...
    /**
     * Get prefered Color Management Module
     *
     * @see getHeaderData()
     * @return string
     */
    ...
    protected function _parseDescTag()
    {
        ...
        $reader = $this->_reader;
        $tagPostition = $this->_tagsData['desc'][0];
        $reader->seek($tagPostition);
        ...
        if ($sig === 'desc') {
            ...
            foreach($recordsData AS $recordData) {
                $reader->seek($tagPostition + $recordData['offset']);
