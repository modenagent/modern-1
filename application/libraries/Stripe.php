...
class Stripe {
	...
	public function __construct( $params ) {
            ...
            if(is_null($params)){
                $this->_conf['stripe_key_test_public']         = 'pk_test_JKTfWhEYh9KIUJhJiD1cI0fo';
                ...
                $this->_conf['stripe_test_mode']               = FALSE;
    ...
    public function cust_info_by_id($customerId){
        try{
            $response = json_decode($this->customer_info($customerId));
        }catch(Exception $e){
        }
    ...
    public function cust_info_by_email($userId, $email,$getcustomerId = false){
        $_CI =& get_instance();
        $_CI->load->model('base_model');
        try{
            $response = json_decode($this->customer_list());
        }catch(Exception $e){
            print_r($e);
        }
        if(isset($response) && isset($response->data)){
            foreach ($response->data as $data){
                ...
                $_CI->base_model->update_record_by_id('lp_user_mst',array('customer_id'=>$data->id),array('user_id_pk'=>$userId));
                if($getcustomerId){
                    return $data->id;
                }
    ...
    public function fetchPlansData($stripeResponseSubs,$insertIntoDB = true){
...
//                if($insertIntoDB)
//                    $this->user_model->savePlaneIfo($userId,$data);
    ...
    function is_subscribed($userId){
        ...
        $_CI->load->model('user_model');
        if($userId){
            $users = $_CI->base_model->get_record_result_array('lp_user_mst',array('user_id_pk' => $userId));
            $customerId = $users[0]['customer_id'];
            if(is_null($customerId) || $customerId == ''){
                $res = $this->cust_info_by_email($userId,$users[0]['email']);
            } else {
                $res = $this->cust_info_by_id($customerId);
            }
            return $res;
        }
        return false;
