 ...
 * rishig may be freely distributed under the MIT license./
 *
...
(function (factory) {
  ...
  var settings = {
    ...
    options: {
      ...
      focus: false,                 // set focus to editable area after initializing rishig
  ...
  var EventHandler = function () {
    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      if ($btn.length) {
        ...
        if ($.isFunction($.rishig.pluginEvents[eventName])) {
          $.rishig.pluginEvents[eventName](event, editor, layoutInfo, value);
    ...
    this.bindKeyMap = function (layoutInfo, keyMap) {
      ...
      $editable.on('keydown', function (event) {
        ...
        if (eventName) {
          if ($.rishig.pluginEvents[eventName]) {
            var plugin = $.rishig.pluginEvents[eventName];
  ...
  var Renderer = function () {
    ...
    /**
     * create rishig layout (normal mode)
     *
     * @param {jQuery} $holder
     * @param {Object} options
     */
  ...
  $.extend($.rishig, {
     ...
     * * layoutInfo is a rishig layout information.
   ...
   * @method addPlugin
   *
   * add Plugin in rishig 
   * 
   * rishig can make a own plugin.
   *
   * ### Define plugin
   * ```
   * // get template function  
   * var tmpl = $.rishig.renderer.getTemplate();
   * 
   * // add a button   
   * $.rishig.addPlugin({
   *     buttons : {
   ...
   * @param {Object} [plugin.events] add event in $.rishig.pluginEvents 
   * @param {Object} [plugin.langs] update $.rishig.lang
   * @param {Object} [plugin.options] update $.rishig.options
   */
  $.rishig.addPlugin = function (plugin) {
    if (plugin.buttons) {
    ...
    if (plugin.events) {
      $.each(plugin.events, function (name, event) {
        $.rishig.pluginEvents[name] = event;
      });
    }

    if (plugin.langs) {
      $.each(plugin.langs, function (locale, lang) {
        if ($.rishig.lang[locale]) {
          $.extend($.rishig.lang[locale], lang);
        }
      });
    }

    if (plugin.options) {
      $.extend($.rishig.options, plugin.options);
    }
  };
  ...
  $.fn.extend({
     ...
     * 
     * ```
     * $("#rishig").rishig( { options ..} );
     * ```
     *   
     * @member $.fn
     * @param {Object} options reference to $.rishig.options
     * @returns {this}
     */
    rishig: function (options) {
      // extend default options
      options = $.extend({}, $.rishig.options, options);

      // Include langInfo in options for later use, e.g. for image drag-n-drop
      // Setup language info with en-US as default
      options.langInfo = $.extend(true, {}, $.rishig.lang['en-US'], $.rishig.lang[options.lang]);
