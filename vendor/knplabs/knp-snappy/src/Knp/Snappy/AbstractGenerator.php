...
abstract class AbstractGenerator implements GeneratorInterface
{
    ...
    protected function executeCommand($command)
    {
        ...
        //$process->run();
        $file = fopen("temp/logs.csv","w");
        while (true) {
                    if (!$process->isStarted()) {
                        //echo "Process starts<br/>";
                        $process->start();
                        continue;
                    }
                    
                    $logText = "".$process->getIncrementalErrorOutput();
                    
                    $info = "";
                    if($logText!==''){
                        $infoPos = strrpos($logText, "]");
                        //var_dump($infoPos);
                        if($infoPos!==false){
                            $info = trim(substr($logText,$infoPos+1));
                            if($info!==''){
                                fputcsv($file,array($info));
                            }
                        }
                    }
                    if (!$process->isRunning()) {
                        //echo "Process stopped<br/>";
                        fclose($file);
                        return [
                            $process->getExitCode(),
                            $process->getOutput(),
